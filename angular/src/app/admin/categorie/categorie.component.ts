...
export class CategorieComponent implements OnInit, OnDestroy {
  ...
  getPage(startIndex, pageSize, sortBy, sortDir, nom,) {
    const sub = this.uow.categories.getAll(startIndex, pageSize, sortBy, sortDir,  nom,).subscribe(
      (r: any) => {
        console.log(r.list);
  ...
  async delete(id: number) {
    ...
    if (r === 'ok') {
      const sub = this.uow.categories.delete(id).subscribe(() => this.update.next(true));
  ...
  displayImage(urlImage: string) {
    ...
    return `${this.url}/categories/${urlImage.replace(';', '')}`;
  }
  ...
  //check box
  //
  isSelected(row: Categorie): boolean {
