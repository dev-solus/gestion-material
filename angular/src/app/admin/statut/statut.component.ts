...
import { Statut } from 'src/app/models/models';
...
export class StatutComponent implements OnInit, OnDestroy {
  ...
  getPage(startIndex, pageSize, sortBy, sortDir, nom,) {
    const sub = this.uow.statuts.getAll(startIndex, pageSize, sortBy, sortDir,  nom,).subscribe(
      (r: any) => {
        console.log(r.list);
  ...
  add() {
    this.openDialog(new Statut(), 'Ajouter statut').subscribe(result => {
      if (result) {
        this.update.next(true);
      }
    });
  ...
  edit(o: Statut) {
    this.openDialog(o, 'Modifier statut').subscribe((result: Statut) => {
      if (result) {
        this.update.next(true);
      }
    });
  ...
  async delete(id: number) {
    const r = await this.mydialog.openDialog('statut').toPromise();
    if (r === 'ok') {
      const sub = this.uow.statuts.delete(id).subscribe(() => this.update.next(true));
  ...
  displayImage(urlImage: string) {
    ...
    return `${this.url}/statuts/${urlImage.replace(';', '')}`;
  }
  ...
  async deleteList() {
    ...
    if (r === 'ok') {
      const sub = this.uow.statuts.deleteRange(this.selectedList.map(e => e.id) as any).subscribe(() => {
        this.selectedList = [];
        this.update.next(true);
      });
